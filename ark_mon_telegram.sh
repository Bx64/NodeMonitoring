...
##
#
# ARK Node check & Telegram alert script
...
. ~/NodeMonitoring/dpos_node_telegram.conf
...
# Telegram CLI1 check (relay) - send notification
#

function telegram_send_cli1
{
...
# Telegram CLI3 check (core) - send notification
#

function telegram_send_cli3
{
...
# Telegram API test, alive/health check & latency - send notification
#

function telegram_send_api
{
...
# Telegram CPU usage - send notification
#
...
# Telegram software version - send notification
#
...
# Telegram HDD capacity - send notification
#
...
# Telegram API blockheight - send notification.
#
...
if [ "$1" == "test" ] ; then
    SEND_ALERT_FLAG_CPU=false
    MESSAGE="$(date) - [TEST] ARK node $HOSTNAME TEST message!"
    echo " >>>> : $MESSAGE"
    telegram_send_api
    exit 0
fi
...
if [[ $ALIVE == "00" ]] ; then
    MESSAGE="$(date) - [SYSTEM] [OK] ARK node $HOSTNAME is alive!"
    echo " >>>> : $MESSAGE"
    telegram_send_api
    exit 0
fi
...
if $PRODUCER == true ; then
    if /home/ark/ark-core/packages/core/bin/run $@ --token=ark forger:status | grep online >/dev/null ; then
        if [ -f "$FILE_CLI2" ] ; then
            ...
            MESSAGE="$(date) - [INFO] [ALERT PRODUCER RESOLVED] ARK node $HOSTNAME producer process is online again!"
            rm $FILE_CLI2
            SEND_ALERT_FLAG_CLI2=true
            telegram_send_cli2
        else
            echo " >>>> : ARK node $HOSTNAME producer process is online!"
        fi
    else
        check_alert_count_cli2
        MESSAGE="$(date) - [CRITICAL] [ALERT PRODUCER] ARK node producer process is not online! #count:$COUNTER - hostname=$HOSTNAME"
        echo " >>>> : $MESSAGE"
        telegram_send_cli2
    fi
else
...
HTTP_CODE=$(curl -s -w '%{http_code}' --connect-timeout 5 --max-time 10 -o /dev/null https://explorer.ark.io/api/peers/$DPOS_NODE_IP)
CURL_STATUS=$?
...
if [ "$CURL_STATUS" -eq 0 ] ; then
    ...
    if [[ "$HTTP_CODE" -ne 200 ]] ; then
        ...
        MESSAGE="$(date) - [CRITICAL] [ALERT] ARK node is not found in API call! #count:$COUNTER - returning http_code=$HTTP_CODE hostname=$HOSTNAME"
        echo " >>>> : $MESSAGE"
        telegram_send_api
    else
        echo " >>>> : ARK node $HOSTNAME is found in API call!"

        STATUS_HEALTHY=$(curl -sS --max-time 50 --retry 20 --retry-delay 2 --retry-max-time 40 https://explorer.ark.io/api/peers/$DPOS_NODE_IP | jq -r '.data.latency')

        if [[ "$STATUS_HEALTHY" -lt $LATENCY_CRITICAL ]] ; then
            MESSAGE="$(date) - [INFO] ARK node is healthy! - LATENCY(ms)=$STATUS_HEALTHY hostname=$HOSTNAME"
            echo " >>>> : $MESSAGE"
#           echo " >>>> : $STATUS_HEALTHY"
            if [ -f "$FILE_API" ] ; then
                echo "$FILE_API exists."
                MESSAGE="$(date) - [INFO] [ALERT RESOLVED] ARK node is healthy again! - LATENCY(ms)=$STATUS_HEALTHY hostname=$HOSTNAME"
                rm $FILE_API
            ...
            check_alert_count_api
            MESSAGE="$(date) - [CRITICAL] [ALERT LATENCY] ARK node is not healthy! #count:$COUNTER - LATENCY(ms)=$STATUS_HEALTHY hostname=$HOSTNAME"
            echo " >>>> : $MESSAGE"
            telegram_send_api
        fi
    fi
    ...
    check_alert_count_api
    MESSAGE="$(date) - [CRITICAL] [ALERT] ARK node is not found in API call! #count:$COUNTER - hostname=$(hostname)"
    echo " >>>> : $MESSAGE"
    telegram_send_api
fi
...
BLOCK_STATUS_ARK=$(curl -sS --max-time 40 --retry 20 --retry-delay 2 --retry-max-time 40 https://explorer.ark.io/api/blockchain | jq '.data.block.height')
BLOCK_STATUS_NODE=$(curl -sS $LOOPBACK:$API_PORT/api/blockchain | jq .'[].block.height')
BLOCKS_BEHIND="$(($BLOCK_STATUS_ARK - $BLOCK_STATUS_NODE))"

if [[ "$BLOCKS_BEHIND" -lt $MAX_BLOCKS_BEHIND ]] ; then
    MESSAGE="$(date) - [INFO] $HOSTNAME Blockchain sync within acceptable limit! - Blockheight at $HOSTNAME only $BLOCKS_BEHIND blocks behind ARK network"
    echo " >>>> : Blockheight at ARK network=$BLOCK_STATUS_ARK"
    ...
    MESSAGE="$(date) - [CRITICAL] [ALERT BLOCKS] $HOSTNAME Blocks are $BLOCKS_BEHIND blocks behind from ARK network! #count:$COUNTER_BLOCKS - Limit is $MAX_BLOCKS_BEHIND blocks for $HOSTNAME"
    echo " >>>> : $MESSAGE"
    telegram_send_blocks
fi
...
SOFTWARE_VERSION=$(/home/ark/ark-core/packages/core/bin/run --token=ark version)
GITHUB_VERSION=$(curl -s https://github.com/ArkEcosystem/core/tags |grep /ArkEcosystem/core/archive/refs/tags/ | head -n 1| cut -d'/' -f7 |awk -F '.zip' '{print $1}')
if [[ "$SOFTWARE_VERSION" == "$GITHUB_VERSION" ]] ; then
    ...
    echo " >>>> : $MESSAGE"
    echo " >>>> : Installed ARK-core version $SOFTWARE_VERSION"
    echo " >>>> : ARK-core release $GITHUB_VERSION at GitHub"
    if [ -f "$FILE_SW" ] ; then
        echo "$FILE_SW exists."
        MESSAGE="$(date) - [INFO] [ALERT RESOLVED] Latest software ARK-core $SOFTWARE_VERSION installed on $HOSTNAME"
        rm $FILE_SW
    ...
    MESSAGE="$(date) - [CRITICAL] [ALERT SOFTWARE VERSION] ARK-cli software is outdated! #count:$COUNTER_SW - INSTALLED VERSION=$SOFTWARE_VERSION GITHUB VERSION=$GITHUB_VERSION hostname=$HOSTNAME"                
    echo " >>>> : $MESSAGE"
    telegram_send_sw
fi
...
if (( $(echo "$CPU_LOAD $CPU_LOAD_CRITICAL" | awk '{print ($1 > $2)}') )) ; then
    ...
    MESSAGE="$(date) - [CRITICAL] [ALERT FIRING] ARK node high CPU usage problem! #count:$COUNTER_CPU - Please check your processess $PROC - Linux SAR total CPU usage: $CPU_LOAD % - hostname=$HOSTNAME"
    ...
    if [ -f "$FILE_CPU" ] ; then
        ...
        MESSAGE="$(date) - [INFO] [ALERT RESOLVED] ARK node normal CPU usage again! - Linux SAR total CPU usage: $CPU_LOAD % - hostname=$HOSTNAME"
        rm $FILE_CPU
